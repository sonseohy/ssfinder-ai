...
def get_args():
    ...
    parser.add_argument('--process-all', action='store_true', help='모든 항목 처리 (중복 방지)')
    parser.add_argument('--batch-size', type=int, default=50, help='배치 처리 크기')
    parser.add_argument('--tracking-file', default='processed_items.json', help='처리된 항목 추적 파일')
...
def main():
    ...
    try:
        ...
        if args.item_id:
            ...
            # 모든 항목 처리 (중복 방지)
            logger.info("모든 항목 처리 중 (중복 방지)...")
            stats = processor.process_all_items(
                args.host, args.port, args.user, args.password, args.database,
                batch_size=args.batch_size, tracking_file=args.tracking_file
            )
