...
def fetch_items_batch(host: str = None, port: int = None, user: str = None, 
                     password: str = None, database: str = None, 
                     limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
    """
    MySQL에서 습득물 데이터를 배치로 가져오는 함수
    
    Args:
        host, port, user, password, database: DB 연결 정보
        limit (int): 가져올 항목 수
        offset (int): 오프셋
    ...
    try:
        ...
        if pool:
            with get_connection() as conn:
                cursor = conn.cursor(dictionary=True)
                _execute_found_items_batch_query(cursor, limit, offset)
                found_items = cursor.fetchall()
        else:
            # 직접 연결
            if not all([host, port, user, password, database]):
        ...
        # 날짜/시간 객체를 문자열로 변환 (JSON 직렬화를 위해)
        for item in found_items:
            if 'found_at' in item and item['found_at']:
                item['found_at'] = item['found_at'].isoformat()
        
        logger.info(f"{len(found_items)}개의 습득물 데이터를 가져왔습니다. (오프셋: {offset})")
...
def _execute_found_items_batch_query(cursor, limit: int, offset: int) -> None:
    """
    습득물 데이터 배치 조회 쿼리를 실행하는 내부 함수
    ...
    SELECT 
        ...
        location, phone, status, found_at, management_id
    FROM 
        found_item
    ORDER BY 
        id DESC
