...
class LostItemAnalyzer:
    def __init__(self):
        ...
        BLIP 기반 분실물 분석기 초기화
        """
        self.device = "cpu"  # GPU 제거, CPU만 사용
    ...
    def _translate_results(self, caption: str, title: str, description: str, 
        ...
        # 특이사항 번역 (여기서는 간단한 키워드 매핑만 사용)
        translated_features = distinctive_features
        
        # 설명 번역
        translated_description = f"{caption}\n\n"  # 원본 캡션은 유지
        
        if material and "unknown" not in material.lower():
            translated_description += f"재질: {translated_material}\n"
            
        if distinctive_features and "none" not in distinctive_features.lower():
            translated_description += f"특이사항: {translated_features}\n"
