...
# FastAPI 애플리케이션 생성
app = FastAPI(
    title="게시글 임베딩 생성 API",
    description="한국어 CLIP 모델을 사용하여 게시글(텍스트 + 이미지)의 임베딩을 생성하는 API",
    version="1.0.0",
)
...
async def root():
    ...
    """
    return {
        "app_name": "게시글 임베딩 생성 API",
        "version": "1.0.0",
        "description": "한국어 CLIP 모델을 사용하여 게시글(텍스트 + 이미지)의 임베딩을 생성합니다.",
        "docs_url": "/docs",
        "api_prefix": "/api"
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='게시글 임베딩 생성 API 서버')
