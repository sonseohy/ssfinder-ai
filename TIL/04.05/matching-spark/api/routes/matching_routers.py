...
from utils.spark_processor import SparkProcessor  # 새로 추가된 SparkProcessor 임포트
...
def get_spark_processor():
    """
    Spark 프로세서 인스턴스를 반환 (싱글톤 패턴)
    """
    global spark_processor
    if spark_processor is None:
        try:
            # Spark 프로세서 초기화 (환경에 맞게 설정 조정)
            spark_processor = SparkProcessor(
                app_name="Post-Similarity",
                master="local[*]",  # 서버 환경에 맞게 조정
                num_clip_services=2  # 사용 가능한 GPU 수에 맞게 조정
            )
            spark_processor.start()
        except Exception as e:
            logger.error(f"Spark 프로세서 초기화 실패: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Spark 프로세서 초기화 실패: {str(e)}")
...
async def find_similar_items_api(
    ...
    use_spark: bool = Query(True, description="Spark를 사용하여 대규모 처리 수행"),
    clip_model=Depends(get_clip_model),
    spark_processor=Depends(get_spark_processor) if use_spark else None
):
    """
    사용자 게시글과 유사한 게시글을 찾는 API 엔드포인트
    
    Args:
        post (LostItemPost): 사용자의 게시글
        threshold (float): 유사도 임계값
        limit (int): 반환할 최대 항목 수
        use_spark (bool): Spark 사용 여부
        clip_model: CLIP 모델 인스턴스 (의존성 주입)
        spark_processor: Spark 프로세서 인스턴스 (의존성 주입)
    ...
    try:
        ...
        # Spark 사용 여부에 따라 처리 방식 선택
        if use_spark and spark_processor:
            logger.info("Spark 기반 대규모 유사도 검색 시작")
            
            # 데이터베이스에서 게시글 데이터 로드 (실제 환경에 맞게 조정)
            posts_df = spark_processor.spark.read.format("jdbc") \
            ...
            # 유사한 게시글 검색
            similar_items = spark_processor.find_similar_posts(
                user_post, 
                posts_df, 
                threshold=threshold, 
                limit=limit
            ...
            logger.info("기존 방식으로 유사도 검색 시작")
            
            # 기존 코드: 데이터베이스에서 게시글 가져오기
            # 실제 환경에서는 데이터베이스 연결 코드로 대체
            from api.database import get_posts  # 가정된 함수
            posts = get_posts(limit=1000)  # 기존 방식으로는 제한된 수의 게시글만 처리
            
            if not posts:
                raise HTTPException(status_code=500, detail="게시글 데이터를 가져오지 못했습니다.")
            
            # 기존 코드: 유사도 계산
            from utils.similarity import find_similar_items
            similar_items = find_similar_items(user_post, posts, threshold, clip_model)
            ...
            matches=[
                {
                    ...
                    "details": {
                        "text_similarity": round(item.get("details", {}).get("text_similarity", 0), 4),
                        "image_similarity": round(item.get("details", {}).get("image_similarity", 0), 4) 
                            if item.get("details", {}).get("image_similarity") else None
                    }
                }
        ...
        return MatchingResponse(
            success=True,
            message=f"{len(similar_items)}개의 유사한 게시글을 찾았습니다.",
            result=result
        )
...
async def create_embeddings_api(
    input_table: str = Query(..., description="입력 테이블 이름"),
    output_table: str = Query(..., description="출력 테이블 이름"),
    jdbc_url: str = Query("jdbc:postgresql://localhost:5432/mydatabase", description="JDBC URL"),
    user: str = Query("username", description="데이터베이스 사용자 이름"),
    password: str = Query("password", description="데이터베이스 암호"),
    batch_size: int = Query(100, description="배치 크기"),
    spark_processor=Depends(get_spark_processor)
):
    """
    게시글 임베딩 생성 API 엔드포인트 (관리자용)
    
    Args:
        input_table (str): 입력 테이블 이름
        output_table (str): 출력 테이블 이름
        jdbc_url (str): JDBC URL
        user (str): 데이터베이스 사용자 이름
        password (str): 데이터베이스 암호
    ...
    try:
        # 임베딩 생성 파이프라인 실행
        spark_processor.create_post_embedding_pipeline(
        ...
        return {
            "success": True,
            "message": f"임베딩 생성 작업이 완료되었습니다. ({input_table} -> {output_table})"
        }
