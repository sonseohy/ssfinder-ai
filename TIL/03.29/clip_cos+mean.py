...
# 하이브리드 유사도 결과 처리 (카테고리별 그룹화 및 정렬)
def process_hybrid_results(similarity_results):
    """
    하이브리드 접근 방식으로 유사도 결과 처리
    - 카테고리별로 그룹화
    - 각 카테고리 내에서 코사인 유사도로 정렬
    ...
    filtered_results = [
        ...
        if result['similarity'] >= TEXT_SIMILARITY_THRESHOLD
    ]
    ...
    for item, result in filtered_results:
        if result['found_category']:
            category = result['found_category']
            category_groups[category].append((item, result))
        else:
            uncategorized.append((item, result))
    
    # 4. 사용자 카테고리와 일치하는 그룹을 우선적으로 처리
    user_category = None
...
def compare_lost_and_found(user_image_path, user_description, num_items=10, analyze_top_n=3):
    ...
    for found_item in processed_items:
        # 유사도 상세 분석 (순수 코사인 유사도 + 카테고리 정보)
        similarity_result = analyze_similarity_detailed(user_item, found_item)
        
        # 결과 저장
        similarity_results.append((found_item, similarity_result))
    
    # 7. 하이브리드 접근으로 결과 처리 (카테고리별 그룹화)
    hybrid_results = process_hybrid_results(similarity_results)
    
    # 8. 결과 출력
    all_sorted = hybrid_results['all_sorted']
    category_groups = hybrid_results['category_groups']
    user_category = hybrid_results['user_category']
    
    print(f"\n총 {len(processed_items)}개 중 {len(all_sorted)}개의 유사한 습득물을 찾았습니다.")
    
    if all_sorted:
        print("\n사용자 카테고리:", user_category if user_category else "알 수 없음")
        ...
        if user_category and user_category in category_groups:
            ...
            for i, (item, result) in enumerate(matching_group[:analyze_top_n]):
                print(f"\n{i+1}. 유사도: {result['similarity']:.4f} (카테고리: {user_category})")
        ...
        for category, items in category_groups.items():
            if category != user_category:  # 사용자 카테고리와 일치하는 그룹은 이미 출력했으므로 건너뜀
                ...
                for i, (item, result) in enumerate(items[:analyze_top_n]):
                    print(f"\n{i+1}. 유사도: {result['similarity']:.4f} (카테고리: {category})")
                    print_item_details(item['original'])
        ...
        # 카테고리를 알 수 없는 아이템 출력
        if hybrid_results['uncategorized']:
            print(f"\n▶ 카테고리를 알 수 없는 아이템: {len(hybrid_results['uncategorized'])}개")
            
            for i, (item, result) in enumerate(hybrid_results['uncategorized'][:analyze_top_n]):
                print(f"\n{i+1}. 유사도: {result['similarity']:.4f} (카테고리: 알 수 없음)")
                print_item_details(item['original'])
    else:
        print("유사한 습득물을 찾지 못했습니다.")
    
    # 9. 시각화 (사용자 이미지가 있을 경우)
    if user_image and all_sorted:
        # 우선 순위: 같은 카테고리 → 높은 유사도
        visual_items = []
        
        # 사용자 카테고리와 일치하는 아이템 추가
        if user_category and user_category in category_groups:
            for item, _ in category_groups[user_category][:5]:
                # 원본 아이템과 이미지 함께 추가
                item_data = item['original'].copy()  # 복사본 사용
                item_data['image'] = item['image']  # 이미지 추가
                visual_items.append(item_data)
        
        # 남은 자리는 유사도 순으로 채움
        remaining_slots = 5 - len(visual_items)
        if remaining_slots > 0:
            used_ids = set(item.get('atcId', '') for item in visual_items)
        ...
        # 유사도 값 추출
        similarity_values = []
        for orig_item in visual_items:
            matching_result = next((result for item, result in all_sorted 
                                if item['original'].get('atcId', '') == orig_item.get('atcId', '')), None)
            if matching_result:
                similarity_values.append(matching_result['similarity'])
            else:
                similarity_values.append(0.0)
        
        visualize_similarities(user_image, visual_items, similarity_values)
