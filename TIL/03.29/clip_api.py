...
# 이미지 가중치 (매우 낮게 설정)
IMAGE_WEIGHT = 0.3  # 이미지보다 텍스트에 70% 가중치

# 카테고리 가중치 설정
CATEGORY_MATCH_BONUS = 0.5  # 카테고리 일치 시 50% 보너스
CATEGORY_MISMATCH_PENALTY = 0.3  # 카테고리 불일치 시 30% 페널티

# 물품 카테고리 매핑 (유사한 물품들을 그룹화)
CATEGORY_MAPPING = {
...
# 브랜드 목록 - 다양한 제품을 생산하는 브랜드는 카테고리 결정에 사용하지 않음
BRAND_LIST = ['삼성', '애플', '엘지', 'LG', '샤오미', '화웨이', '소니', '파나소닉', 
              '나이키', '아디다스', '퓨마', '리복', '뉴발란스', 
              '구찌', '루이비통', '샤넬', '프라다', '에르메스']

# 특정 브랜드 제품명 - 이 키워드가 포함되면 해당 카테고리로 간주
SPECIFIC_PRODUCTS = {
    '휴대폰': ['갤럭시폰', '아이폰', '픽셀폰', '갤럭시휴대폰', '갤럭시폴드', '갤럭시노트', '갤럭시플립', '갤럭시 z'],
    '태블릿': ['아이패드', '갤럭시탭', '갤탭', 'ipad', 'galaxy tab'],
    '노트북': ['맥북', '갤럭시북', 'macbook', 'galaxy book'],
...
def check_category_match(category1, category2):
    ...
    Args:
        category1 (str): 첫 번째 카테고리 또는 None
        category2 (str): 두 번째 카테고리 또는 None
        
    Returns:
        int: 1 (일치), 0 (알 수 없음), -1 (불일치)
    """
    # 둘 중 하나가 None이면 판단 불가
    if category1 is None or category2 is None:
        return 0  # 중립 (보너스나 페널티 없음)
...
def cosine_similarity(embedding1, embedding2):
    """
    두 임베딩 벡터 간의 코사인 유사도 계산 (차원 불일치 체크 추가)
    """
    ...
    if embedding1.shape != embedding2.shape:
        print(f"경고: 임베딩 차원 불일치 - embedding1: {embedding1.shape}, embedding2: {embedding2.shape}")
        # 차원이 다른 경우, 더 작은 차원에 맞춰 자르기
        min_dim = min(embedding1.shape[0], embedding2.shape[0])
        embedding1 = embedding1[:min_dim]
        embedding2 = embedding2[:min_dim]
        print(f"임베딩을 {min_dim}차원으로 조정했습니다.")
    
    # 임베딩이 비정상적인 경우 (NaN, Inf 등) 체크
    if not np.all(np.isfinite(embedding1)) or not np.all(np.isfinite(embedding2)):
        print("경고: 임베딩에 NaN 또는 Inf 값이 포함되어 있습니다.")
        # NaN이나 Inf 값을 0으로 대체
    ...
    if norm1 == 0 or norm2 == 0:
        print("경고: 영벡터가 감지되었습니다. 유사도를 0으로 반환합니다.")
        return 0.0
...
def check_image_text_consistency(item_image_embedding, item_text_embedding):
    ...
    consistency = cosine_similarity(item_image_embedding, item_text_embedding)
    return consistency

# 아이템에서 카테고리 정보 추출
def analyze_similarity_detailed(user_item, found_item):
    """
    사용자 분실물과 습득물 간의 유사도를 상세히 분석 (카테고리 처리 개선)
    """
    ...
    # 텍스트 유사도 (텍스트-텍스트)
    text_text_sim = cosine_similarity(user_item['text_embedding'], found_item['text_embedding'])
    print(f"텍스트-텍스트 유사도: {text_text_sim:.4f}")
    
    # 이미지가 있는 경우에만 이미지 관련 유사도 계산
    img_img_sim = 0.0
    if user_item['image_embedding'] is not None and found_item['image_embedding'] is not None:
        # 이미지 유사도 (이미지-이미지)
        img_img_sim = cosine_similarity(user_item['image_embedding'], found_item['image_embedding'])
        print(f"이미지-이미지 유사도: {img_img_sim:.4f}")
    ...
    user_text = user_item.get('text', '')
    user_category_field = None  # 사용자 입력에는 카테고리 필드가 없음
    ...
    # 카테고리 추출
    user_category = extract_category(user_text)
    ...
    # 카테고리 매칭 확인
    category_match_result = check_category_match(user_category, found_category)
    if category_match_result == 1:
        print("카테고리 일치: 같은 종류의 물품입니다.")
    elif category_match_result == -1:
        print("카테고리 불일치: 다른 종류의 물품입니다.")
    else:
        print("카테고리 확인 불가: 카테고리 정보가 충분하지 않거나 '기타' 카테고리입니다.")
    
    # 최종 유사도 점수
    final_score = text_text_sim
    
    # 이미지 유사도 반영 (매우 낮은 가중치)
    if img_img_sim > 0:
        final_score = (final_score * (1 - IMAGE_WEIGHT)) + (img_img_sim * IMAGE_WEIGHT)
    
    # 카테고리 일치/불일치 보너스/페널티 적용
    if category_match_result == 1:
        # 카테고리 일치 시 보너스
        final_score *= (1 + CATEGORY_MATCH_BONUS)
        print(f"카테고리 일치 보너스 적용: {CATEGORY_MATCH_BONUS * 100}%")
    elif category_match_result == -1:
        # 카테고리 불일치 시 페널티
        final_score *= (1 - CATEGORY_MISMATCH_PENALTY)
        print(f"카테고리 불일치 페널티 적용: -{CATEGORY_MISMATCH_PENALTY * 100}%")
    else:
        print("카테고리 보너스/페널티 없음: 중립적인 평가")
    
    print(f"최종 유사도 점수: {final_score:.4f}")
    print("===== 유사도 분석 완료 =====\n")
...
def compare_lost_and_found(user_image_path, user_description, num_items=10, analyze_top_n=3):
    ...
    # 6. 각 습득물에 대해 유사도 점수 계산 및 카테고리 매칭 적용
    similarity_results = []
    
    for found_item in processed_items:
        # 사용자 분실물과 습득물의 유사도 계산 (카테고리 보너스/페널티 포함)
