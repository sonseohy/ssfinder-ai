...
def extract_keywords(text):
    ...
    # 불용어 제거 (필요시 한국어 불용어 리스트 추가)
    # stopwords = ['은', '는', '이', '가', '을', '를', '에', '의', '과', '와', '으로', '로', '에서']
...
def calculate_text_similarity(text1, text2, weights=None):
    ...
    if common_words:
        ...
        pos_diff_sum = sum(abs(positions1[word] - positions2[word]) for word in common_words)
...
def calculate_similarity(user_post, lost_item, clip_model=None):
    ...
    # 5. 장소 유사도 (선택적)
    location_sim = 0.0
    if 'location' in user_post and 'location' in lost_item:
        location_sim = calculate_text_similarity(user_post['location'], lost_item['location'])
    text_similarities['location'] = location_sim
    ...
    # 위치 정보가 있는 경우 추가 가중치 부여 (선택적)
    if 'location' in user_post and 'location' in lost_item:
        text_similarity = text_similarity * 0.9 + location_sim * 0.1
